import { Wine } from "@/types/myprofileTypes";
import Image from "next/image";
import images from "../../public/images/images";
import HamburgerMenu from "./HamburgerMenu";
import { MyProfileCard } from "./MyProfileCard";

interface MyWineCardProps {
  id: number;
  wine: Wine;
  teamId: string | null;
  token: string | null;
  tab: string;
  openId: number | null;
  setOpenId: (id: number | null) => void;
  onDeleteSuccess: () => void;
}

export default function MyWineCard({ ...props }: MyWineCardProps) {
  return (
    <MyProfileCard
      pt="pt-[30px]"
      mb="mb-[62px]"
      className="gap-[40px] h-[228px] items-end"
    >
      {/* 와인 이미지 */}
      <div className="w-[76px] h-[300px] overflow-hidden">
        <Image
          src={props.wine.image || images.defaultProfile}
          alt="와인이미지"
          width={76}
          height={270}
          className="w-full h-full translate-y-[24px] object-cover"
        />
      </div>

      {/* 텍스트 정보 */}
      <div className=" flex flex-col items-start h-full ">
        <div className="text-[#2D3034] font-bold text-[30px] mb-[20px]">
          {props.wine.name}
        </div>
        <div className="text-[#9facbd] text-sm mb-[13px]">
          {props.wine.region}
        </div>
        <div className="flex items-center bg-palepink text-garnet px-[15px] py-[6px] rounded-[12px] w-max">
          <div className="font-bold text-md">
            ₩ {props.wine.price.toLocaleString()}
          </div>
        </div>
      </div>

      {/* 햄버거 메뉴 */}
      <div className="h-full ml-auto">
        <HamburgerMenu {...props} />
      </div>
    </MyProfileCard>
  );
}
